    ...
    public IEnumerator ScopeRun()
    {
        while (begin)//repeat the routine as long as begin is true
        {
            ...
            for (int i = 0; i < Panels.Count; i++)//run through every panel
            {
                ...
                if (i == 1 || i == 2)
                {
                    textStep.GetComponent<Text>().text = "Albert and Carl are not directly related so return false.";
                }
                else if (i == 4 || i == 5)
                {
                    textStep.GetComponent<Text>().text = "Check Who Albert's child is.  Return Bob";
                }
                else if (i == 6 || i == 7 || i == 8 || i == 9)
                {
                    textStep.GetComponent<Text>().text = "Check if Bob is related to Carl.";
                }
                else
                {
                    textStep.GetComponent<Text>().text = "";
                }
